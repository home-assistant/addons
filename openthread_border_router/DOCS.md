...
| upgrade_threshold  | Router upgrade threshold (default: 8). The number of router-capable End Devices (REEDs) that must be present before a REED attempts to become a Thread Router. Lower values promote REEDs to routers sooner, increasing mesh density. |
| downgrade_threshold | Router downgrade threshold (default: 24). The number of Thread Routers that must be present before a Router attempts to downgrade to REED. Higher values keep more routers in the mesh, improving redundancy at the cost of higher radio overhead. |
