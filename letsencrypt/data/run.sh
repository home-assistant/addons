...
if [[ "$EMAIL" == "hello@home-assistant.io" ]] || [[ "$DOMAINS" == "home-assistant.io" ]]; then
  bashio::log.error "Please update the Email and Domain section with your details"
  exit
fi
...
#Google
    ...
    export GOOGLE_CREDS
    PROVIDER_ARGUMENTS=("--${DNS_PROVIDER}" "--${DNS_PROVIDER}-credentials" /ssl/"$GOOGLE_CREDS")
#All others
else
